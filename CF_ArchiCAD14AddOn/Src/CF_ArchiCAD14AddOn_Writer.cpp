...
//------------------------------------------------------------------------------
WRL::MAWriter::MAWriter (const API_IOParams& ioParams, GSModeler::Model* maModel):
	outputFile (NULL),
	model (NULL),
	...
	nMaterials (0)
{
	...
	try {
		...
		outputFile = new IO::File (*ioParams.fileLoc, IO::File::Create);
...
//------------------------------------------------------------------------------
void		WRL::MAWriter::WriteTexCoordAdd (bool isLast, GSModeler::TextureCoordinate* uvCoord)
{
	char	buffer[256];
