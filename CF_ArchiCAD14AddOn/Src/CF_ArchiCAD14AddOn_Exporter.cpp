...
// Constructor
//------------------------------------------------------------------------------
WRL::MAExporter::MAExporter (GSModeler::Model* newModel, const API_IOParams* ioParams) :
	model (NULL),
	writer (NULL)
{
	...
	try {
		writer = new MAWriter (ioParameters, model);
	}
...
void		WRL::MAExporter::CreateTextures (void)
{
	...
	IO::Name	fName = *ioParameters.saveFileIOName;
	fName.DeleteExtension ();		// if any
	fName.Append ("_");
	fName.Append (localizedStrings[WRL::TexturesName].ToCStr ());

	textureLoc = *ioParameters.fileLoc;
...
void		WRL::MAExporter::CreateTextureFile (Int32 textureIndex, char* fullPathName)
{
...
#if 0
	GSModeler::Texture::Pixel* pixelMap = new GSModeler::Texture::Pixel[texture.GetPixelMapSize ()];
	...
	BNCopyMemory (pixMapPtr, pixelMap, texture.GetPixelMapSize () * sizeof (GSModeler::Texture::Pixel));
#endif
	...
	if (name[0] == '\0')
		sprintf (fileName, "%s%ld.jpg", localizedStrings[WRL::TextureName].ToCStr (), (GS::LongForStdio) textureIndex);
	else
	...
	sprintf (fullPathName, "%s%c%s", pathBuffer, WRL::Delimiter, fileName);
}
...
{
	writer->WriteHeader (const_cast<char*> (localizedStrings[WRL::CreatedText].ToCStr ()));
...
void		WRL::MAExporter::CalcSceneLimits (Coord3D* minCoord, Coord3D* maxCoord)
{
	...
	GSModeler::Element			element;
	GSModeler::Body				body;
	GSModeler::Polygon			polygon;
	GSModeler::ConvexPolygon	convexPolygon;
	GSModeler::Vertex			vertex;
...
{
	GSModeler::Light	light;
	double				intensity;
	Int32				nLights = model->GetLightNum ();

	for (Int32 i = 1; i <= nLights; i++) {
		model->GetLight (i, &light);

		switch (light.GetType ()) {
			case GSModeler::Light::DirectionLight:
			case GSModeler::Light::SunLight:
			...
			case GSModeler::Light::SpotLight:
			...
			case GSModeler::Light::PointLight:
...
// Export a body
//------------------------------------------------------------------------------
void		WRL::MAExporter::ExportBody (GSModeler::Body& body)
{
	...
	GSModeler::ConvexPolygon		convexPolygon;
	GSModeler::Material				material;
	GSModeler::Polygon				polygon;
	GSModeler::TextureCoordinate	uvCoord;
	GSModeler::Vector				polygonNormal;
	GSModeler::Vector				vertexNormal;
	GSModeler::Vertex				vertex;
	...
	for (iPgon = 1; iPgon <= nPgon; iPgon++) {
		...
		// IndexedFaceSet
		writer->WriteIndexedFaceSetBegin (body.IsClosed ());
		...
		if (smooth) {
			...
			if (iPgon == 1) {
				Int32**	normalIndices = reinterpret_cast<Int32**> (BMhAllClear ((body.GetVertexNum () + 1) * sizeof (Int32)));
				if (BMError () != NoError)
					throw GS::OutOfMemoryException ();
				...
				try {
					for (iConvexPgon = 1; iConvexPgon <= nConvexPgon; iConvexPgon++) {
						...
						for (iPedge = 1; iPedge <= nPedge; iPedge++)
							(*normalIndices)[convexPolygon.GetVertexIndex (iPedge)] = convexPolygon.GetNormalVectorIndex (iPedge);
					}
				}
				catch (const GS::Exception& e) {
					e.Print (dbChannel);
				...
				for (j = 1; j <= nVertex; j++) {
					bool	hasNormal = ((*normalIndices)[j] > 0);
					if (hasNormal)
						body.GetVector ((*normalIndices)[j], &vertexNormal);

					if (hasNormal && (fabs (vertexNormal.x) + fabs (vertexNormal.y) + fabs (vertexNormal.z) > EPS))
						writer->WriteVectorListAdd (j == nVertex, j - 1, vertexNormal.x, vertexNormal.y, vertexNormal.z);
					else
						writer->WriteVectorListAdd (j == nVertex, j - 1, 0.0, 0.0, 1.0);
				}
				BMhKill (reinterpret_cast<GSHandle*> (&normalIndices));
			}
		...
		try {
			for (iConvexPgon = 1; iConvexPgon <= nConvexPgon; iConvexPgon++) {
				...
				for (iPedge = 1; iPedge <= nPedge; iPedge++) {
					body.GetVertex (convexPolygon.GetVertexIndex (iPedge), &vertex);
					try {
						polygon.GetTextureCoordinate (&vertex, &polygonNormal, &uvCoord);
					}
					catch (...) {
		...
		if (material.GetTextureIndex () > 1)
			CHTruncate (textureNames[material.GetTextureIndex () - 2].ToCStr(), textureName, WRL::MaxStringLength);
...
void		WRL::MAExporter::ExportBodies (void)
{
	...
	short	phaseNum = 1;
	ACAPI_Interface (APIIo_InitProcessWindowID, (void*) localizedStrings[WRL::SavingFile].ToCStr (), &phaseNum);
	ACAPI_Interface (APIIo_SetNextProcessPhaseID, (void*) "", &nElements);
...
{
	char	name[WRL::MaxStringLength];

	strcpy (name, "");	// dummy string
	localizedStrings.Add (name);

	RSGetIndString (name, WRL::LocalizedStringTable, WRL::CreatedText);
	localizedStrings.Add (name);

	RSGetIndString (name, WRL::LocalizedStringTable, WRL::TextureName);
	localizedStrings.Add (name);

	RSGetIndString (name, WRL::LocalizedStringTable, WRL::TexturesName);
	localizedStrings.Add (name);

	RSGetIndString (name, WRL::LocalizedStringTable, WRL::SavingFile);
	localizedStrings.Add (name);
}
